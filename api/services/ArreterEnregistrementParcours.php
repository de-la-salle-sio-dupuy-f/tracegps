    ...
    if ( $pseudo == "" || $mdpSha1 == "") {
        ...
        if ( $dao->getNiveauConnexion($pseudo, $mdpSha1) == 0 ) {
            ...
            if($uneTrace==null){
                ...
                $unIdUtilisateurParTrace = $dao->getUneTrace($idTrace)->getIdUtilisateur();
                if ($unIdUtilisateur != $unIdUtilisateurParTrace){
                    $msg = "Erreur : le numéro de trace ne correspond pas à cet utilisateur.";
                    $code_reponse = 400;
                }
                else{
                    $terminee = $dao->getTerminee();
                    if($terminee == 1){
                        $msg = "Erreur : cette trace est déjà terminée.";
                        $code_reponse = 400;
                    }
                    else{
                        $terminee = setTerminee($terminee);
