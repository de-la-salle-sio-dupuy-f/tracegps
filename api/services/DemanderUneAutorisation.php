...
// Rôle : ce service permet à un utilisateur de demander son mot de passe
...
$mdp = ( empty($this->request['pseudo'])) ? "" : $this->request['pseudo'];
...
{	$msg = "Erreur : méthode HTTP incorrecte.";
    ...
    if ( $pseudo == "" || $mdp == "" || $pseudoDestinataire == "" || $texteMessage == "" || $nomPrenom == "" ) {
        ...
        $mdpVraie = $dao->$unUtilisateur->getMdpSha1();
        if($mdp != $mdpVraie || $unUtilisateur == null){
            $msg = "Erreur : authentification incorrecte.";
            $code_reponse = 400;
        }
        
        else {
            $unUtilisateurExistant = $dao->getUnUtilisateur($pseudoDestinataire);
            if($unUtilisateurExistant == null){
                $msg =  "Erreur : pseudo utilisateur inexistant.";
                $code_reponse = 400;
            }
            else {
                // envoie un courriel  à l'utilisateur avec son nouveau mot de passe
                $ok = $dao->envoyerMdp ($pseudo, $mdp);
                if ( ! $ok ) {
